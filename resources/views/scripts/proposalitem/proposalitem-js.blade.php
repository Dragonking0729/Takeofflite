<script>
    ...
    function fetch_data(page) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                document.getElementById("add_proposalitem_button").dataset.page = page;
            }
        });
    }
    ...
    function updateProposalItem() {
        if ($('#takeoff_uom').val() === " ") {
            ...
            if (!data.item_desc) {
                ...
                someBlock.preloader();
                $.ajax({
                    url: "{{ url('proposal_item/update_proposalitem') }}",
                    type: 'POST',
                    data: {
                        data: data,
                        _token: _token
    ...
    const url = <?php echo json_encode(route('proposal_item.store')); ?>; // url
    const someBlock = $('.someBlock');
    let origin_proposalitem_number;
    let origin_proposalgroup_number;
    let addProposalItemFlag = false;
    ...
    $(document).on('click', '#add', function () {
        $("#default_proposal_item_section").hide();
        $("#add_proposal_item_section").show();
        $("#add_proposal_item").trigger("reset");
        addProposalItemFlag = true;
    });
    ...
    // add cancel
    $(document).on('click', '.cancel', function () {
        $("#default_proposal_item_section").show();
        $("#add_proposal_item_section").hide();
        $("#add").prop('disabled', false);
        addProposalItemFlag = false;
        updateCancelBtnClicked();
    });
    ...
    $(document).on('click', '.save', function () {
        if (!$("#aproposal_group").val()) {
            toastr.error("Please select proposal group");
            ...
            let page = document.getElementById('add_proposalitem_button').dataset.page;
            let formData = $("#add_proposal_item").serializeArray();
            // get formula
            someBlock.preloader();
            $.ajax({
                url: "",
                ...
                success: function (data) {
                    ...
                    if (data.status === 'error') {
                        ...
                        $("#update").hide();
                        $("#back").show();
                        $('#proposalitem_tree').jstree(true).settings.core.data = JSON.parse(data.tree_data);
                        $('#proposalitem_tree').jstree(true).refresh();
                        fetch_data(page);
                    }
                }
            });
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (res.status === 'success') {
                            ...
                            $('#default_proposal_item_section').html(res.data.view_data);
                            $('#proposalitem_tree').jstree(true).settings.core.data = JSON.parse(res.data.tree_data);
                            $('#proposalitem_tree').jstree(true).refresh();
                            if (page === 1) {
                                fetch_data(page);
                            } else {
    ...
    $(document).on('click', '#renumber', function () {
        origin_proposalitem_number = $('#item_number').val();
        origin_proposalgroup_number = $('#proposalgroup').val();
        let group_number = $('#proposalgroup').data('group_number');

        $('#proposalgroup').val(group_number);
        $('#proposalgroup').attr('type', 'number');
    ...
    $(document).on('click', '#renumber_confirm_button', function () {
        let id = $(this).data('id');
        let updated_proposalitem_number = $('#item_number').val();
        ...
        someBlock.preloader();
        $.ajax({
            url: "{{ url('proposal_item/renumbering') }}",
            type: 'POST',
            data: {
                id: id,
                updated_proposalitem_number: updated_proposalitem_number,
                updated_proposalgroup_number: updated_proposalgroup_number,
                proposalgroup_number: origin_proposalgroup_number,
                _token: _token
            },
            success: function (data) {
                someBlock.preloader('remove');
    ...
    $(document).on('click', '#renumber_confirm_cancel_button', function () {
        ...
        $('#item_number').val(origin_proposalitem_number);
        $('#proposalgroup').val(origin_proposalgroup_number);
    });


    /**
     * Add proposal item dropdown - atakeoff_uom, alabor_uom, amaterial_uom, asubcontract_uom
     */
    ...
    /**
     * Update proposal item dropdown - takeoff_uom, labor_uom, material_uom, subcontract_uom
     */
    ...
    // resizable modal
    let $proposalitem_treeview = $('#proposalitem_treeview');
    $proposalitem_treeview.find('.modal-content')
        .resizable({
            handles: 'n, e, s, w, ne, sw, se, nw'
        });


    let $create_proposalitem_treeview = $('#create_proposalitem_treeview');
    $create_proposalitem_treeview.find('.modal-content')
        .resizable({
            handles: 'n, e, s, w, ne, sw, se, nw'
        });
