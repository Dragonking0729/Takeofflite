<script>
    ...
    // testing formula
    $(document).on('click', '#open_assembly_item_formula_test_modal', function () {
        inputConstant();
        if (tags.length) {
            ...
            $('#assembly_item_formula_test_modal').modal({backdrop: 'static', keyboard: false});
    ...
    // testing formula when add
    $('#test_formula_add').click(function () {
        inputConstant();
        if (newTags.length) {
            $('#test_formula_result_div').html('');
            let parsedResult = null;
            ...
            let result = genFormulaForm(globalItemVal, globalItemFormula, false);
            $('#parsed_formula_div').html(result.formula);
            $('#test_formula_body').html(result.html);
            $('#assembly_item_formula_test_modal').modal({backdrop: 'static', keyboard: false});
    ...
    $("#assembly_item_formula_test_modal").on('keyup change', '#test_formula_body input', function () {
        refreshModal();
    });
