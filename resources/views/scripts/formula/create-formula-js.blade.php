<script>
    ...
    $(document).on('select2:select', '#add_variables', function() {
        ...
        if (questionType === 'standard') {
            let found = newTags.some(tag => tag.questionType === questionType);
            if (found || newTags.length === 0) {
                let cursorPos = getCurrentCorsurPosAdd();
                let variable = {type: 'variable', questionType: questionType, val: selected.trim()};
                newTags.splice(cursorPos, 0, variable);
                addNewTags(cursorPos+1);
            } else {
                toastr.error('That question type is not compatible with a formula');
            }
        } else {
            if (newTags.length) {
                toastr.error('That question type is not compatible with a formula');
    ...
    $(document).on('click', '.add_operators', function() {
        ...
        addNewTags(cursorPos+1);
        updateBtnClicked();
        // let found = newTags.some(tag => tag.questionType === 'standard');
        // if (!newTags.length || !found) {
        //     toastr.error('Combining formula with this operator not allowed');
        // } else {
        //     if (newTags[0].val === 'IF' && newTags[newTags.length-1].val === ')') {
        //         tastr.error('Now allowed');
        //     } else {
        //         inputConstant();
        //         let cursorPos = getCurrentCorsurPosAdd();
        //         let selected = $(this).data('value');
